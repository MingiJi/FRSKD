...
class BiFPN(nn.Module):
    def __init__(self, network_channel, num_classes, args):
        ...
        self.net_channels = [args.num_channels] * len(network_channel)
        for i in range(repeat):
            self.layers.append(BiFPN_layer(i == 0, DepthConvBlock, network_channel, num_channels, depth))
...
class BiFPN_layer(nn.Module):
    def __init__(self, first_time, block, network_channel, num_channels, depth):
        ...
        for i, channels in enumerate(network_channel):
            if self.first_time:
                self.lat_conv.append(block(channels, num_channels, 1, 1, 0, lat_depth))

            if i != 0:
                self.lat_conv2.append(block(channels if self.first_time else num_channels, num_channels, 1, 1, 0, lat_depth))
                self.down_conv.append(block(num_channels, num_channels, 3, 1, 1, down_depth))
            ...
            if i != len(network_channel) - 1:
                self.up_conv.append(block(num_channels, num_channels, 3, 1, 1, up_depth))
